...
exports.handler = async (req, res) => {
  try {
    ...
    // Insert a new record into the book_issue_requests table
    const insertResult = await queryDatabase(
      connection,
      "INSERT INTO book_issue_requests (user_id, book_id, accession) VALUES (?, ?, ?)",
      [userid, bookid, accession]
    );

    if (insertResult.affectedRows === 1) {
      // Book issue request successfully inserted
      return res.status(200).send("Book issue request sent to admin.");
    } else {
      // Failed to insert the request
      return res.status(500).send("Failed to send book issue request.");
    }
  } catch (error) {
